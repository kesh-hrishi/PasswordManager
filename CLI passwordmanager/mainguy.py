...
parser.add_argument("--length",help="Length of the password to genrate",type=int)
...
def main():
	if args.option in ["add","a"]:
		...
		if res is not None:
			newentry.addEntry(res[0],res[1],args.name,args.url,args.email,args.login)
	...
	if args.option in ["extract","e"]:
		...
		if res is not None:
			getvalues.retrieveEntries(res[0],res[1],search,decryptPassword = args.copy)
